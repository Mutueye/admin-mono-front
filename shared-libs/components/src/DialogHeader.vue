<template>
  <div class="flex flex-row items-start justify-between w-full">
    <div class="flex flex-col justify-start items-start">
      ...
      <div v-if="subTitle" class="text-size-12px leading-14px mt-8px" :class="subTitleDanger ? 'color-danger' : 'color-text-secondary'">
        {{ subTitle }}
      </div>
    </div>
    <div class="flex flex-row items-center">
      ...
      <button
        ...
        <div class="w-20px h-20px transform-gpu relative rotate-45 group">
          <div class="w-2px h-14px rounded absolute bg-text-secondary left-1/2 top-1/2 -mt-7px -ml-1px group-hover:bg-primary" />
          <div class="w-14px h-2px rounded absolute bg-text-secondary left-1/2 top-1/2 -mt-1px -ml-7px group-hover:bg-primary" />
        </div>
      </button>
    </div>
  </div>
